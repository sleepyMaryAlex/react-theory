...
Другими словами, React ждет, пока весь код в обработчиках событий не будет запущен, прежде чем обрабатывать ваши обновления состояния. Это позволяет вам обновлять несколько переменных состояния — даже из нескольких компонентов — без запуска слишком большого количества повторных рендерингов.

Такое поведение, также известное как пакетная обработка, позволяет вашему приложению React работать намного быстрее. Это также позволяет избежать сбивающих с толку «недоделанных» рендеров, в которых были обновлены только некоторые переменные.
...
Здесь `(actualCount) => actualCount + 1` называется функцией обновления. Когда вы передаете его установщику состояния:

1. React ставит эту функцию в очередь для обработки после выполнения всего остального кода в обработчике событий.
2. Когда вы вызываете `useState`, во время следующего рендеринга React проходит через очередь и выдает вам окончательное обновленное состояние.

Функции обновления запускаются во время рендеринга, поэтому функции обновления должны быть чистыми и возвращать только результат.
...
Если вы предпочитаете более подробный код, другим распространенным соглашением является повторение полного имени переменной состояния, например:
...
Состояние, хранящееся в React, могло измениться к моменту запуска `alert`, но оно было запланировано с использованием моментального снимка состояния на момент взаимодействия с ним пользователя!

Значение переменной состояния никогда не меняется внутри рендера.
